...
function Scene3D(aParentObj, azIndex, aWidth, aHeight) {
	...
	this.Zoom = new Vector(1.0, 1.0, 1.0);     //zoom 变焦，BoundingBox对象的单位向量，基标准。
	this.OrderWeight = new Vector(1.0, 1.0, 1.0);
	this.ZoomAll = 1.0;
	this.ShiftX = 0.0;
	this.ShiftY = 0.0;
	if (useSVG) {
		this.XM = aWidth / 2;    //Scene3D图片宽度占svg的的一半；
		this.YM = aHeight / 2;   //Scene3D图片高度占svg的的一半；
	} else {
		this.XM = 250;
		this.YM = 250;
	}
...
/*初始化Scene3D对象，在scene.svg中添加两个子标签*/
function _Scene3DInit() {
	if (useSVG) {
		this.BoxGroup = this.Parent.GetSVGDoc().createElementNS(  //createElementNS创建一个带有指定命名空间的元素节点，返回一个Element对象；与createElement区别就是创建的时候也指定了命名空间；
				"http://www.w3.org/2000/svg", "g");
		this.BoxGroup.setAttribute("x", "0");                     //节点中添加属性值
		this.BoxGroup.setAttribute("y", "0");
		this.BoxGroup.setAttribute("width", this.XM * 2);
		this.BoxGroup.setAttribute("height", this.YM * 2);
		this.Parent.GetScene().appendChild(this.BoxGroup);        //在scene.svg中的id为Scene的元素中的最后一个子节点的后边添加子节点，也就是上边创建的g标签
...
function _Scene3DAddPoly(oo) {
	...
	if (useSVG) {
		this.Shape[ii] = this.Parent.GetSVGDoc().createElementNS(  //在scene.svg中创建路径节点，闭合的路径，来绘制Poly3D对象的空间显示图像。
				"http://www.w3.org/2000/svg", "path");
		this.Shape[ii].setAttribute("style","stroke-width:1.2;stroke:#CD950C;stroke-opacity:0.9;"); //修改小方格的边框颜色 等样式
 	//    this.Shape[ii].setAttribute("style","stroke-width:1.2;stroke:black;stroke-opacity:0.9;");
...
function Poly3D(aParentScene, aFrontColor, aBackColor, aStrokeColor,
	...
	this.PhPoint = new Array();   //Poly3D对象的各个顶点的向量组成的数组（基本的坐标）。本页422行可以看出。
	this.Point = new Array();     //Poly3D对象的各个顶点的向量在Scene3D中的坐标，组成的数组。
	this.Center = new Vector(0.0, 0.0, 0.0);  //
    ...
    this.SetEventAction=_Object3DSetEventAction1;
    this.Id = "";
	this.text = "";
	this.Callback = new Array();
	this.Parent.AddPoly(this);           //调用Scene3D对象中的AddPoly方法，将该Poly3D对象添加进去。
}
...
function _Poly3DDraw(aShape) {
	...
	if (useSVG) {
		...
		if (this.Parent.Parent.GetScene().getAttribute("first") != 1) {
			...
			text2.setAttribute("font-size", "11.5px");
		  //text2.setAttribute("fill","#BF3EFF");               //插入的文本的样式-颜色
     		text2.setAttribute("fill","#030303");
			//text2.setAttribute("onmouseover",par);
			//text2.attachEvent("onmouseover", function(){alert("df");});
			//text2.onmouseover=parent.show2;
			//text2.onmouseout=parent.hid2;
			//text2.addEventListener("onmouseover",parent.test, false);
		...
		if(text=="undefined"){
			aShape.nextSibling.textContent = "*";  
		}else{
			aShape.nextSibling.textContent = text;  
		}                           //将温度 以文本格式显示在每个Poly3D对象上边。
       ...
       /*设置数字背景颜色*/
		if ((ll >= 3) && (this.FrontColor != ""))
			if (Number(text) <= 10)
				aShape.setAttribute("fill", this.Parent.GetColor("#00B5E9",this.BackColor, this.Normal, this.Center));
			    //aShape.setAttribute("fill", url('abc.gif'), this.Normal, this.Center));
			else if (Number(text) <= 20)
				aShape.setAttribute("fill", this.Parent.GetColor("#96FF00",this.BackColor, this.Normal, this.Center));
			else if (Number(text) <= 30)
				aShape.setAttribute("fill", this.Parent.GetColor("#FFE400",this.BackColor, this.Normal, this.Center));
			else if (Number(text) >30)
				aShape.setAttribute("fill", this.Parent.GetColor("#FF0000",this.BackColor, this.Normal, this.Center));
			else
				aShape.setAttribute("fill", this.Parent.GetColor(this.BackColor,this.BackColor, this.Normal, this.Center));
		//
		if (this.StrokeColor)
...
/*日期：2014 1 29*/
function _Object3DSetEventAction1(aEvent, aCallback) {
	if (aCallback){
		this.Parent.Callback[aEvent] = aCallback;
		this.Callback[aEvent] = aCallback;
	}
}
