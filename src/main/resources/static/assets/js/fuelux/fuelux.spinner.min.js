    ...
    a.prototype = {
        constructor: a, render: function () {
            ...
            if (this.options.precision!=null){
                this.$input.attr("precision", this.options.precision + "")
            }
        }, change: function () {
            var d = this.$input.val();
            var precision=parseInt(this.$input.attr("precision"));
            if (d / 1) {
                //判断有无超出最小值
                if (parseFloat(d)<parseFloat(this.$input.attr('min'))) {
                    d=this.$input.attr('min')
                    d=parseFloat(d);
                    this.$input.val(d.toFixed(precision));
                }
                //判断有无超出最大值
                if (parseFloat(d)>parseFloat(this.$input.attr('max'))) {
                    d=this.$input.attr('max')
                    d=parseFloat(d);
                    this.$input.val(d.toFixed(precision));
                }
                //判断小数位是否超出
                if (d.toString().indexOf(".")>0&&d.toString().split(".")[1].length>precision) {
